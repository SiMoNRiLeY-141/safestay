...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (prevRoomsRef.current.length > 0) {
      const newEmergencies = rooms.filter(
        (room) =>
          room.guestStatus === "Need Help" &&
          prevRoomsRef.current.find((r) => r.id === room.id)?.guestStatus !==
            "Need Help"
      );
      ...
      if (newEmergencies.length > 0) {
        try {
          ...
          const playBeep = (timeOffset: number) => {
            ...
            gainNode.gain.exponentialRampToValueAtTime(
              0.01,
              audioCtx.currentTime + timeOffset + 0.2
            );
        ...
        newEmergencies.forEach((room) => {
          ...
          setNotifications((prev) => [
            ...prev,
            { id, room: room.name, type: room.helpType || "Unknown Emergency" },
          ]);
          setTimeout(() => {
  ...
  const handleResetStatuses = () => {
    if (
      confirm("Are you sure you want to reset all room statuses to 'Unknown'?")
    ) {
      resetStatuses();
    }
  };
  ...
  const handleDeleteSelected = () => {
    if (
      confirm(
        `Are you sure you want to delete ${selectedRooms.length} selected room(s)?`
      )
    ) {
  ...
  const handleAskAI = async () => {
    ...
    setAiResponse(null);
    try {
      const roomDataStr = rooms
        .map(
          (r) =>
            `Room ${r.name} (Floor: ${r.floor || "Ground Floor"}) - Status: ${r.guestStatus}${r.helpType ? ` - Need: ${r.helpType}` : ""}`
        )
        .join("\n");
    ...
    <main className="min-h-screen bg-transparent p-4 sm:p-6 transition-colors duration-300">
      ...
      <div className="fixed top-24 right-4 z-50 flex flex-col gap-3 pointer-events-none w-72 sm:w-80">
        {notifications.map((n) => (
          <div
            key={n.id}
            className="pointer-events-auto bg-red-50 dark:bg-[#050b14]/90 border-2 border-red-500 rounded-xl p-4 shadow-[0_0_20px_rgba(220,38,38,0.5)] animate-in slide-in-from-right-full fade-in duration-300 cyber-glitch backdrop-blur-md"
          >
          ...
          <div className="flex flex-wrap xl:flex-nowrap items-center gap-2 sm:gap-3 w-full lg:w-auto lg:justify-end">
            ...
            <button
              ...
              className={`text-[10px] md:text-xs px-3 py-1.5 rounded-lg font-bold tracking-wider uppercase transition-all border shrink-0 ${
                isGenerating
                  ? "bg-purple-100 dark:bg-purple-900/40 text-purple-400 dark:text-purple-500 border-purple-300 dark:border-purple-800 cursor-not-allowed"
                  : "bg-purple-100 dark:bg-purple-900/60 text-purple-700 dark:text-purple-300 border-purple-400 dark:border-purple-500 hover:bg-purple-200 dark:hover:bg-purple-800 hover:shadow-[0_0_15px_rgba(168,85,247,0.3)] dark:hover:shadow-[0_0_15px_rgba(168,85,247,0.5)]"
              }`}
            >
            ...
            <button
              ...
              className={`text-[10px] md:text-xs px-3 py-1.5 rounded-lg font-bold tracking-wider uppercase transition-all border shrink-0 ${
                activeTab === "dashboard"
                  ? "bg-cyan-600 text-white border-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.6)]"
                  : "bg-cyan-100 dark:bg-cyan-900/40 text-cyan-800 dark:text-cyan-400 border-cyan-400 dark:border-cyan-800 hover:bg-cyan-200 dark:hover:bg-cyan-800 hover:text-cyan-900 dark:hover:text-cyan-200"
              }`}
            >
              Dashboard
            </button>
            <button
              ...
              className={`text-[10px] md:text-xs px-3 py-1.5 rounded-lg font-bold tracking-wider uppercase transition-all border shrink-0 ${
                activeTab === "property"
                  ...
                  : "bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-400 border-blue-400 dark:border-blue-800 hover:bg-blue-200 dark:hover:bg-blue-800 hover:text-blue-900 dark:hover:text-blue-200"
              }`}
            >
              Property Info
            </button>
            {activeTab === "dashboard" && (
              <button
                ...
                className={`text-[10px] md:text-xs px-3 py-1.5 rounded-lg font-bold tracking-wider uppercase transition-all border shrink-0 ${
                  isEditMode
                    ? "bg-cyan-600 text-white border-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.6)]"
                    : "bg-cyan-100 dark:bg-cyan-900/40 text-cyan-800 dark:text-cyan-400 border-cyan-400 dark:border-cyan-800 hover:bg-cyan-200 dark:hover:bg-cyan-800 hover:text-cyan-900 dark:hover:text-cyan-200 hover:shadow-[0_0_15px_rgba(0,150,255,0.4)] dark:hover:shadow-[0_0_15px_rgba(6,182,212,0.4)]"
                }`}
              >
            ...
            {!isEditMode && (
              <div className="grid grid-cols-3 gap-3 sm:gap-6 mb-8">
                <div className="cyber-panel rounded-xl p-4 sm:p-5 text-center border-t-4 border-t-cyan-500 dark:border-t-cyan-600">
                  <p className="text-2xl sm:text-3xl font-bold text-cyan-700 dark:text-cyan-400 font-mono">
                    {unknownCount}
                  </p>
                  <p className="text-xs sm:text-sm text-cyan-800 dark:text-cyan-600 font-bold uppercase tracking-widest mt-1">
                    Unknown
                  </p>
                </div>
                <div className="cyber-panel rounded-xl p-4 sm:p-5 text-center border-t-4 border-t-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                  ...
                  <p className="text-xs sm:text-sm text-emerald-500 font-bold uppercase tracking-widest mt-1">
                    Safe
                  </p>
                </div>
                <div className="cyber-panel rounded-xl p-4 sm:p-5 text-center border-t-4 border-t-red-600 shadow-[0_0_20px_rgba(220,38,38,0.3)]">
                  <p className="text-2xl sm:text-3xl font-bold text-red-500 font-mono">
                    {helpCount}
                  </p>
                  <p className="text-xs sm:text-sm text-red-500 font-bold uppercase tracking-widest mt-1">
                    Need Help
                  </p>
                </div>
              </div>
            ...
            {isEditMode ? (
              <div className="cyber-panel rounded-2xl p-5 sm:p-6 mb-8 border border-cyan-500/20 shadow-[0_0_24px_rgba(0,240,255,0.08)]">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-bold text-cyan-800 dark:text-cyan-300 font-[family-name:var(--font-orbitron)] uppercase tracking-wider">
                    Room Configuration
                  </h2>
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                      {rooms.map((room) => (
                        <tr
                          key={room.id}
                          className="border-b border-cyan-100/60 dark:border-cyan-900/30 hover:bg-cyan-50/60 dark:hover:bg-cyan-950/30 transition-colors"
                        >
                          <td className="py-3 px-4">
                            <input
                              type="checkbox"
                              aria-label={`Select room ${room.name}`}
                              title={`Select room ${room.name}`}
                ...
                <p className="text-sm text-cyan-800 dark:text-cyan-500 mt-6 border-t border-cyan-200/70 dark:border-cyan-900/50 pt-4">
                  <strong className="text-cyan-900 dark:text-cyan-300">
                    Tip:
                  </strong>{" "}
                  Use Grid Column and Row to visually position rooms on the
                  dashboard. Leaving them empty (&quot;Auto&quot;) will place
                  rooms sequentially in any available spots.
                </p>
              </div>
            ) : (
              /* Live Room Grid */
              <div className="flex flex-col gap-6 sm:gap-8">
                {floors.map((floor) => {
                  const floorRooms = rooms.filter(
                    (r) => (r.floor || "Ground Floor") === floor
                  );
                  const floorMaxCol = Math.max(
                    10,
                    ...floorRooms.map((r) => r.gridCol || 1)
                  );
                  return (
                    <div key={floor}>
                      <h2 className="text-xl sm:text-2xl font-bold text-cyan-800 dark:text-cyan-300 mb-4 font-[family-name:var(--font-orbitron)] tracking-wider uppercase border-b border-cyan-300 dark:border-cyan-800/50 pb-2 inline-block shadow-[0_4px_10px_rgba(6,182,212,0.0)]">
                        {floor}
                      </h2>
