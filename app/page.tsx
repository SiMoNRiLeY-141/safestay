    ...
    <main className="min-h-screen bg-transparent flex flex-col items-center justify-center px-4 py-8 sm:py-10 transition-colors duration-300 relative">
      ...
      <div className="w-full max-w-md cyber-panel border-t-2 border-t-cyan-500/50 rounded-2xl p-6 sm:p-8 flex flex-col items-center gap-6 transition-all duration-300 relative overflow-hidden">
        ...
        <div className="text-center relative z-10">
          <h1 className="text-3xl sm:text-4xl font-black text-cyan-700 dark:text-cyan-400 tracking-[0.15em] font-[family-name:var(--font-orbitron)] uppercase drop-shadow-sm dark:drop-shadow-[0_0_10px_rgba(0,240,255,0.5)]">
            SafeStay
          </h1>
        ...
        <div className="w-full">
          ...
          <div className="relative">
            ...
            >
              {Array.from(new Set(rooms.map((r) => r.floor || "Ground Floor")))
                .sort()
                .map((floor) => (
                  <optgroup key={floor} label={floor}>
                    {rooms
                      .filter((r) => (r.floor || "Ground Floor") === floor)
                      .map((room) => (
        ...
        {!showEmergencyOptions ? (
          ...
          <div className="w-full flex flex-col gap-3 animate-in fade-in duration-300 mt-2">
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-sm font-bold text-cyan-800 dark:text-cyan-400 uppercase tracking-widest font-mono">
                What kind of help?
              </h2>
              <button
              ...
              {EMERGENCY_OPTIONS.map((opt) => (
                <button
                  ...
                  className={`group flex items-center gap-3 p-3.5 rounded-2xl border backdrop-blur-md transition-all active:scale-[0.98] shadow-sm hover:shadow-md ${
                    opt.intensity === "high"
                      ? "bg-gradient-to-br from-red-50/90 to-red-100/60 dark:from-red-900/40 dark:to-red-900/20 border-red-200/50 dark:border-red-800/50 text-red-900 dark:text-red-100 hover:border-red-300 dark:hover:border-red-700"
                      : opt.intensity === "medium"
                        ? "bg-gradient-to-br from-orange-50/90 to-orange-100/60 dark:from-orange-900/40 dark:to-orange-900/20 border-orange-200/50 dark:border-orange-800/50 text-orange-900 dark:text-orange-100 hover:border-orange-300 dark:hover:border-orange-700"
                ...
                >
                  <div
                    className={`p-2 rounded-xl bg-white/10 shadow-[inset_0_1px_2px_rgba(255,255,255,0.2)] transition-transform group-hover:scale-110`}
                  >
                    {opt.icon}
                  </div>
                  <span className="text-xs sm:text-sm font-bold uppercase tracking-wider leading-tight flex-1 text-left">
                    {opt.label}
                  </span>
                </button>
              ))}
        ...
        {submitted && selectedRoomObj && (
          <div
            className={`w-full text-center py-3 px-4 rounded-xl font-bold text-white border backdrop-blur-md shadow-[0_0_15px_rgba(0,0,0,0.5)] ${
              submitted === "Safe"
                ? "bg-emerald-500/80 border-emerald-400"
                : "bg-red-600/80 border-red-500 cyber-glitch"
            }`}
          >
            {submitted === "Safe"
